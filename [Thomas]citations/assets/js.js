...
function getRandomNumber(){
    nombreA = Math.floor(Math.random()*citations.length);
    return nombreA;
}

hasard();

bouton.addEventListener("click",hasard);

function hasard(){
    let cita = getRandomNumber();
    if (cita == save){
        hasard();
    } else {
        h1.textContent = citations[cita][0];
        h2.textContent = citations[cita][1];
        save = cita;
    }
}